...
### Added
- **Runtime Invariant Health Check System (V2-INV-001)** — Deterministic, read-only invariant validation system that continuously verifies economic and ledger correctness across six invariant categories (PR #1138, #1139):
  - **`GET /health/invariants`** — Returns `200 OK` when all invariants pass; `500` with a sanitized failure report when any invariant fails. Suitable for Pulsetic uptime monitoring and Grafana alerting. Exception details are scrubbed from the HTTP response (CodeQL #156).
  - **Ledger ↔ BalanceCache consistency** — Verifies that each student's `BalanceCache` totals match the sum of all non-voided ledger transactions within the same `join_code`.
  - **Idempotency key uniqueness** — Confirms no two transactions share a duplicate idempotency key within a class economy.
  - **Balance rules** — Savings never negative; checking balance negative only when overdraft protection is explicitly enabled.
  - **Transaction state validity** — No unknown `status` values exist in the ledger.
  - **Temporal integrity** — `posted_at` is never earlier than the transaction's creation timestamp.
  - **Money supply integrity** — Aggregate cache equals aggregate ledger; transfer pairs sum to zero and carry correct sign symmetry; per-class supply breakdown; `delta_cents` metric exposed for Grafana.
  - **`transfer_correlation_id`** (String 36, nullable, indexed) added to `transaction` so transfer debit/credit pairs share a UUID. All call sites in `student.py` (×3), `api.py` (×2), and `admin.py` (×3) updated. Legacy NULL rows fall back to a per-(student, join_code) consistency check.
  - **Balance cache reconciliation script** — `scripts/reconcile_balance_cache.py` for one-time or ad-hoc reconciliation of stale `BalanceCache` entries.
...
- **Tiered insurance setup and guidance** — Refactored insurance products to follow a modular tiered design (PR #1090, #1093):
  - Simple, advanced, and custom configuration modes with contextual guidance cards.
  - Preset insurance mode behaviour restored; switching between modes no longer breaks form state (PR #1095).
  - Insurance approval cap handling corrected for variable-amount policies (PR #1099).
  - Economy snapshots integrated into insurance pricing to ensure premium recommendations reflect live class economy conditions (PR #1097).
  - Lock-in workflow enforces spec-compliant validation before a policy becomes purchasable; workflow specification documented in `docs/FEATURES/INSURANCE/FEAT-INS-001_Insurance_Mechanics_and_Workflow.md` (PR #1096).
- **Sysadmin escalated issue "In Review" status** — Added `DEV_IN_REVIEW` as a valid status for escalated issues, allowing sysadmins to signal active investigation before resolution. Sysadmin view for escalated issues renders the new status without a 404 (commit 5425297).
...
- **Economy health and banking admin UX refinements** — Improved economy health dashboard with cleaner layout, corrected aggregation that was double-counting per-class metrics, and refined banking admin controls (PR #1125). Mobile formatting fixed for insurance table and economy health page (PR #1124).
- **Rent waivers: analytics event logging and perk grant suppression** — Rent waivers now emit `analytics_event` records so economy-wide event timelines capture waiver grants. Perk grants (hall passes, store items) are suppressed for waived students so rent-linked perks are not duplicated (PR #1108).
- **Economy policy scheduling refinement and CWI health controls** — Policy application now evaluates scheduling correctness before committing rebalance changes; CWI health thresholds made configurable per-class (PR #1105).
- **Gunicorn structured JSON access logging** — New `gunicorn_json_logging.py` module emits machine-readable JSON access log entries with fields: `timestamp` (local timezone, configurable via `GUNICORN_ACCESS_LOG_TIMEZONE`, defaults to America/Los_Angeles), `method`, `path`, `status`, `response_length`, `duration_ms`. Timezone resolution falls back gracefully to UTC when the configured zone is unavailable. Configuration file `gunicorn-logconfig.json` wires the custom logger into Gunicorn's access log handler. Grafana dashboard `deploy/grafana/dashboards/economy-health.json` added for economy health observability (PR #1157).
- **V1 Rent Stabilization — hardened payment flow** — Comprehensive stability hardening of the rent payment path so failures are visible and scoped rather than silent or partially applied (PR #1156, FEAT-RENT-004):
  - **Centralized payment validation**: `_filter_valid_rent_payments()` replaces 6+ inline timestamp-tolerance match implementations, eliminating drift between call sites.
  - **Pre-insert guard**: remaining-amount check (`remaining_amount <= 0`) is evaluated immediately before the database insert, closing the race-condition window for zero-value or duplicate payments.
  - **Atomic transaction boundary**: `rent_pay()` wrapped in an explicit database transaction with rollback on any unhandled failure; no partial writes (Transaction + RentPayment + hall-pass top-off + StudentItem grant are committed atomically or not at all).
  - **Duplicate submission soft guard**: idempotency check prevents a double-submit from creating a second `RentPayment` row within the same request window.
  - **Anomaly logging**: suspicious payment conditions (zero amount, unexpected state transitions) are logged as `ErrorEvent` records for operator review.
- **Template design system unification** — Replaced legacy Bootstrap icon usage (`bi-*`) with Material Symbols; removed deprecated `bg-opacity-*` utilities in favour of semantic `bg-*-subtle` backgrounds; normalized shell and standalone templates to consistent token-driven theming (PR #1121).
- **Combined Dependabot dependency updates** — Bumped 7 packages in a single pass: `gunicorn` 25.1.0 → 25.3.0, `Pygments` 2.19.2 → 2.20.0, `tomli` 2.4.0 → 2.4.1, `Werkzeug` 3.1.6 → 3.1.7, `cryptography` 46.0.5 → 46.0.6, `redis` 7.3.0 → 7.4.0, `requests` 2.32.5 → 2.33.0. Additional safe upgrades: `pytz`, `qrcode`, `redis`, `cryptography` 46.0.7 (PR #1101, #1135).
- **CI: Production SSH via Tailscale** — Deploy workflow now routes SSH connections through the Tailscale overlay network for improved reliability (PR #1136).

### Fixed
- **Authenticated-route cache now targets the correct sysadmin endpoint** — Corrected the sysadmin route used by authenticated-route caching so cached lookups resolve against the proper system-admin page instead of a mismatched route.
...
- **Rent waivers now honored** — `RentWaiver` records were stored but never consulted when determining rent paid status. A new `_has_active_rent_waiver` helper is called from `_is_student_coverage_period_paid`; any student with an active waiver that spans the coverage due date is immediately treated as paid.
...
- **Rent waiver UI class switching** — The rent waiver form's class selector now correctly reloads waiver data for the newly selected class instead of displaying stale data from the previous selection (PR #1157).
- **Rent full-payment mode** — Fixed a regression where the full-payment flow incorrectly blocked students who had already paid the full amount for the period, preventing access to rent-linked perks (PR #1141).
- **Insurance claim time-limit gate uses filing timestamp** — The claim submission deadline is now evaluated against the filing timestamp (when the claim was submitted) rather than the current wall-clock time, preventing clock drift from retroactively invalidating claims. Teachers can override the deadline on a per-claim basis (PR #1102).
- **Collective goal progress resets on reactivation** — When a teacher reactivates a deactivated collective goal store item, the purchase progress counter now correctly resets to zero (voided `StudentItem` records excluded), rather than carrying over stale counts from before deactivation (PR #1110).
- **Economy health dashboard duplicate aggregation** — Economy health stats were double-counted due to a query that joined the same data source twice; corrected to a single aggregation pass so per-class totals are accurate (PR #1157).
- **Economy health, waivers, and sysadmin logging consistency** — Fixed a batch of smaller inconsistencies: waiver edge cases when no prior payment exists, sysadmin action log entries for economy-health corrections, transaction description formatting for reversal entries (PR #1109).
- **Transaction idempotency hardened** — Prevents duplicate ledger entries from concurrent requests by adding idempotency checks before inserting Transfer and Payroll transaction pairs (PR #1100).
- **Explicit teacher PII deletion with cascading cleanup** — Teacher account deletion now explicitly cascades through all dependent tables in FK-safe order (`student_blocks`, `analytics_*`, `feature_settings`, `payroll_*`, `rent_*`, `banking_settings`, `hall_pass_*`, `store_*`, `insurance_*`, `tap_events`, `transaction`, `balance_cache`, `student_teachers`) before removing the `admins` row, preventing `ForeignKeyViolation` errors that previously surfaced when a teacher with full data was deleted (PR #1157).
- **Invariant ledger totals scoped to class economy** — Money-supply invariant now aggregates ledger and cache totals exclusively within each class's own economy boundary; cross-class totals that previously inflated delta metrics are excluded (PR #1139).
