...
- **Hall-pass available types identity surface** — `/api/hall-pass/available-types` now supports `join_code` and `teacher_public_id` as primary selectors and scopes class lookup to the active student class context. Numeric `teacher_id` remains only as backward-compatible fallback.
