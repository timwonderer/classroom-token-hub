...
- **Runtime Invariant Health Check System (V2-INV-001)** — Deterministic, read-only invariant validation system that continuously verifies economic and ledger correctness across six invariant categories (PR #1138, #1139):
  - **`GET /health/invariants`** — Returns `200 OK` when all invariants pass; `500` with a sanitised failure report when any invariant fails. Suitable for Pulsetic uptime monitoring and Grafana alerting. Exception details are scrubbed from the HTTP response (CodeQL #156).
  ...
  - **`transfer_correlation_id`** (String 36, nullable, indexed) added to `transactions` so transfer debit/credit pairs share a UUID. All call sites in `student.py` (×3), `api.py` (×2), and `admin.py` (×3) updated. Legacy NULL rows fall back to a per-(student, join_code) consistency check.
...
- **Sysadmin escalated issue "In Review" status** — Added `in_review` as a valid status for escalated issues, allowing sysadmins to signal active investigation before resolution. Sysadmin view for escalated issues renders the new status without a 404 (commit 5425297).
...
- **V1 Rent Stabilization — hardened payment flow** — Comprehensive stability hardening of the rent payment path so failures are visible and scoped rather than silent or partially applied (PR #1156, FEAT-RENT-004):
  - **Centralised payment validation**: `_filter_valid_rent_payments()` replaces 6+ inline timestamp-tolerance match implementations, eliminating drift between call sites.
...
- **Template design system unification** — Replaced legacy Bootstrap icon usage (`bi-*`) with Material Symbols; removed deprecated `bg-opacity-*` utilities in favour of semantic `bg-*-subtle` backgrounds; normalised shell and standalone templates to consistent token-driven theming (PR #1121).
...
- **Explicit teacher PII deletion with cascading cleanup** — Teacher account deletion now explicitly cascades through all dependent tables in FK-safe order (`student_blocks`, `admin_invite_codes`, `analytics_*`, `feature_settings`, `payroll_*`, `rent_*`, `banking_settings`, `hall_pass_*`, `store_*`, `insurance_*`, `tap_events`, `transactions`, `balance_cache`, `class_memberships`, `student_teachers`) before removing the `admins` row, preventing `ForeignKeyViolation` errors that previously surfaced when a teacher with full data was deleted (PR #1157).
