# Codex Development Guide for Classroom Token Hub

This document provides essential guidance for Codex (or any AI assistant) working on the Classroom Token Hub project. Following these guidelines ensures consistency, prevents common pitfalls, and maintains the project's high standards.
...
2. **ALWAYS scope database queries by `class_id` (or `join_code`)**
   ...
   - See `.Codex/rules/multi-tenancy.md` for detailed rules
...
**See `.Codex/rules/database-migrations.md` for complete workflow.**
...
**See `.Codex/rules/documentation.md` for complete standards.**
...
**See `.Codex/rules/testing.md` for complete requirements.**
...
**See `.Codex/rules/security.md` for complete guidelines.**
...
**See `.Codex/rules/multi-tenancy.md` for complete rules.**
...
**Solution:** Follow `.Codex/rules/database-migrations.md` exactly. Run the migration linter before committing.
...
- **Detailed Rules:** `.Codex/rules/` for in-depth guidance
...
- **Multi-tenancy** → `.Codex/rules/multi-tenancy.md` and `docs/security/MULTI_TENANCY_AUDIT.md`
...
**Branch:** `codex/v2.0`
**For:** Codex and AI assistants working on Classroom Token Hub
