...
def test_calculate_payroll(client):
    ...
    assert tb.id in payroll_summary
    assert payroll_summary[tb.id] == expected_payroll
    ...
    assert tb.id not in payroll_summary2
...
def test_calculate_payroll_ignores_other_class_manual_payment_anchor(client):
    ...
    summary_a = calculate_payroll_breakdown(class_a.class_id, [tb_a.id], now - timedelta(days=1))
    summary_b = calculate_payroll_breakdown(class_b.class_id, [tb_b.id], now - timedelta(days=1))

    expected_class_b_only = Decimal("3.75")
    assert summary_a == {}
    assert summary_b == {tb_b.id: expected_class_b_only}
...
def test_get_cached_payroll_with_meta(client):
    ...
    assert tb.id in summary
    assert summary[tb.id] > 0
    # Store initial values to compare later
    initial_amount = summary[tb.id]
    ...
    assert str(tb.id) in cache.cached_breakdown
    ...
    assert summary_cached[tb.id] == initial_amount 
    ...
    assert summary_fresh[tb.id] > initial_amount 
