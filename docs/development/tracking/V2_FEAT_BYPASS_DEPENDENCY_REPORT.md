...
- **`get_side_effect`** — a bypass-hidden flush happened inside a GET handler. This is an [INV-ARC-007](../v2_restructure_doc/INVARIANT/ARCHITECTURE/INV-ARC-007_GET_MUST_BE_PURE.md) candidate: GETs are required to be side-effect free. Separate category from dead-route since the fix is "remove the write" rather than "add `@feat_shell`".
