---
title: Student Dashboard Overview
category: features
subcategory: student-account
roles: [student]
description: Understand the student dashboard summary and shortcuts.
keywords: [dashboard, overview]
related:
  - user-guides/features/student/banking/accounts-transfers
  - user-guides/features/student/work/attendance-history
---

# Student Dashboard Overview

Your dashboard summarizes balances, work status, and quick links.

## Where to work
Go to **Dashboard** from the student sidebar.

## What you will see
- Current balances and recent activity.
- Work and pay status.
- Quick links to Store and Bills.

## Related guides
- [Accounts and Transfers](/docs/user-guides/features/student/banking/accounts-transfers)
- [Attendance History and Pay Status](/docs/user-guides/features/student/work/attendance-history)
