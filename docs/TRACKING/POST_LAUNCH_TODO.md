...
|---|---|---|---|---|
| PL-CI-01 | P1 | **Constitutional CI secrets fix is written but not merged.** Every Dependabot PR fails Constitutional CI because Dependabot runs receive no Actions or environment secrets and the empty `ENCRYPTION_KEY` crashes conftest. | PR #1392 (branch `claude/serene-lalande-e14b1d`); dispatched runs 34922923925 and 34927042475 green. Safe to land before launch. | Merge #1392. Then: delete the now-unreferenced `testing` environment secrets (`AUDIT_HMAC_KEY`, `ENCRYPTION_KEY`, `PEPPER_KEY`, `SECRET_KEY`), and comment `@dependabot rebase` on one open Dependabot PR to confirm it goes green. |
...
| ID | Pri | Item | Evidence | Next step |
|---|---|---|---|---|
| PL-OPS-01 | P1 | **`DOM-OPS-001` event tables are not built.** None of the eight in §5 exists: `operational_events`, `audit_log`, `invariant_run_events`, `incident_events`, `incident_summary`, `alert_events`, `job_events`, `health_check_events`. Telemetry still goes to application logs. | `docs/DOMAIN/DOM-OPS-001_OPERATIONS_DOMAIN.md` §5; `app/services/operational_event_service.py:26-50`; `app/services/tlcp.py:86`. The sysadmin crash this causes is a **launch** item, tracked in the readiness launch checklist. **Column contract mismatch:** §5 names the event time `timestamp`, but both reads in `app/services/operational_event_service.py:53-86` select and order by `created_at`, and no model or alias maps one to the other. A table built to §5 would make both sysadmin reads fail on an undefined column. | Build per §5, then retire the log-file reads in the sysadmin views. Align the timestamp in the same change: either move both service queries, and every consumer of the row's `created_at` key (the sysadmin dashboard and combined-logs views and their templates), to `timestamp`, or alias `timestamp AS created_at` in the service. **Closes when** the tables exist per §5 and an authenticated sysadmin GET of `/sysadmin/dashboard` and `/sysadmin/combined-logs` renders against them, with a regression test. |
