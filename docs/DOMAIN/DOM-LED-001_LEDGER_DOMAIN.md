...
- **INV-LED-015: Seat Balance Serialization**. Every operation that reads a seat's available balance before creating a balance-affecting transaction, and every settlement for that seat, MUST acquire an exclusive lock on the scoped `seats` row. The lock order is deterministic: `seats` row first, then the scoped `ClassEconomy` row, then pending transactions and balance snapshots. This prevents debits from racing settlement's posted/pending transition and makes the seat row the single serialization point for seat money.
