...
16. Disabling the creation of new obligation state MUST NOT make surviving lawful obligation state impossible to resolve. While rent is disabled, succession and new rent assessment stop; everything needed to resolve what already exists continues: existing obligations remain viewable and payable, and grace and late-fee rules keep applying to them under the policy version each is frozen to. Resolving the last surviving obligation does not re-enable rent or schedule a successor. A feature toggle is never a debt-forgiveness mechanism.
